...
{
    return sqrt(pow(p1.x-p2.x,2)+pow(p1.y-p2.y,2));//改了
}
...
armor::armor(const LED_Stick& L1, const LED_Stick& L2)
{
    Led_stick[0]= L1;
    Led_stick[1]= L2;
    error_angle = fabs(L1.rect.angle - L2.rect.angle);//计算误差角
    rect.width = abs(static_cast<int>(L1.rect.center.x - L2.rect.center.x));//装甲板矩形宽高
    rect.height = static_cast<int>((L1.rect.size.height + L2.rect.size.height)/2);
    center.x = static_cast<int>((L1.rect.center.x + L2.rect.center.x)/2);//装甲板中心坐标
    center.y = static_cast<int>((L1.rect.center.y + L2.rect.center.y)/2);
    rect.x = center.x - rect.width/3;//装甲板矩形
 ...
 *
 * @param img
 * @param roi_offset_point
 */
void armor::draw_rect( Mat& img, Point2f roi_offset_point) const
{
    rectangle(img, rect + Point_<int>(roi_offset_point), Scalar(255,255,255), 1);//img
}

void armor::draw_spot(Mat &img, Point2f roi_offset_point) const
{
    circle(img, center + Point_<int>(roi_offset_point), int(rect.height/4), Scalar(0,0,255), -1);//图片，圆心，圆半径，圆颜色，圆轮廓厚度，圆线类型
}
...
/**
 * 此处用于判断装甲板尺寸是否合适的参数都可修改，尤其是长宽比判断
 * @return
 */
...
bool Armordetector::DetectArmor(Mat &img,Rect rect_roi) {
    //二值化

    Mat roi_image = img(rect_roi);
    Point2f offset_roi_point(rect_roi.x, rect_roi.y);
    vector<LED_Stick> LED_Stick_v;  // 声明所有可能的灯条容器
    Mat binary_brightness_img;
    Mat binary_color_img;
    Mat gray;
    cvtColor(roi_image, gray, COLOR_BGR2GRAY);//灰度
    vector<cv::Mat> bgr;
    split(roi_image, bgr);//通道分离
    Mat result_img;
    ...
    //cv::Mat channels[3];
    //cv::split(roi_image,channels); //三通道分离
    if (color_ == 0) {
        subtract(bgr[2], bgr[1], result_img);
        //result_img=channels[2]-channels[0];
        }
    else{
        subtract(bgr[0], bgr[1], result_img);
        //result_img=channels[1]-channels[2];
    }

    threshold(gray, binary_brightness_img, 100, 255, CV_THRESH_BINARY);//color_th
    threshold(result_img, binary_color_img, 100, 255, CV_THRESH_BINARY);//grow_th

//膨胀
    dilate(binary_brightness_img,erodeSrc,struct1);
    //dilate(binary_brightness_img,erodeSrc,struct1);
    dilate(binary_color_img,erodeSrc1,struct1);
    //dilate(binary_color_img,erodeSrc1,struct1);
    floodFill(binary_brightness_img,Point(5,50),Scalar(255),0,FLOODFILL_FIXED_RANGE);
    floodFill(binary_color_img,Point(5,50),Scalar(255),0,FLOODFILL_FIXED_RANGE);
    threshold(gray, binary_brightness_img, 100, 255, CV_THRESH_BINARY_INV);//color_th
    threshold(result_img, binary_color_img, 100, 255, CV_THRESH_BINARY_INV);//grow_th
    imshow("binary_brightness_img",binary_brightness_img);
    imshow("binary_color_img",binary_color_img);

#ifdef SHOW_end_IMAGE

#endif

    vector<vector<Point>> contours_light;
    vector<vector<Point>> contours_brightness;
    findContours(binary_color_img, contours_light, RETR_EXTERNAL, CHAIN_APPROX_NONE);//查找轮廓
    ...
    for (size_t i = 0; i < contours_brightness.size(); i++) {
        double area = contourArea(contours_brightness[i]);//计算轮廓面积
        if (area < 20.0 || 1e5 < area) continue;
        for (size_t ii = 0; ii < contours_light.size(); ii++) {
            //pointPolygonTest根据轮廓进行点检测，measureDist-false时，返回值是1、-1、0。否则返回值是点到最近的轮廓边缘之间的有符号的距离
            if (pointPolygonTest(contours_light[ii], contours_brightness[i][0], false) >= 0.0) {
                ...
                if (length > 10 && length < 4000) {           // 使用拟合椭圆的方法要比拟合最小矩形提取出来的角度更精确
                    RotatedRect RRect = fitEllipse(contours_brightness[i]);
/**#ifdef SHOW_CONTOURS_LIGHT
                    //旋转矩形提取四个点
                    ...
                    putText(img, to_string(RRect.angle), RRect.center + Point2f(2,2) + offset_roi_point, FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,255), 1);//画轮廓
#endif
                    if (fabs(RRect.angle) <= 45)  // 超过一定角度的灯条不要
                    {
                        LED_Stick r(RRect);
...
// **分类装甲板** -根据灯条匹配状态得到最终装甲板
    ...
    armor target;
    Point2f roi_center(rect_roi.width/2, rect_roi.height/2);
    float dx,dy;
    ...
    rectangle(img, rect_roi,Scalar(0, 255, 255),1);//roi区域的矩形
#endif
...
//            circle(img, points_2d_.at(i), 3, Scalar(i*50, i*50, 255), -1);
    ...
    */
    ...
    rectangle(img, last_target_,Scalar(0,125,125), 1);//在图像中绘制矩形
    lost_cnt_ = 0;
}else {
//计算ROI的相关参数
lost_cnt_ ++;
